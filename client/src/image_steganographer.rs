...
pub trait ImageSteganographer: Send + Sync {
    ...
    fn decode(&self, encoded_image: &[u8], decoded_image_path: &str) -> Result<Vec<u8>, String>;
}
...
impl ImageSteganographer for SomeImageSteganographer {
    ...
    fn encode(&self, secret_image: &[u8], output_path: &str) -> Result<Vec<u8>, String> {
        ...
        let temp_secret_path = "/tmp/secret_image.png";
        ...
        let carrier_path = "/home/magdeldin@auc.egy/Cloud-P2P-environment/service_provider/carrier.jpg";
        ...
        let secret_bytes = steganography::util::file_to_bytes(secret_image);
