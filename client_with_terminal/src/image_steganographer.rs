...
impl ImageSteganographer for SomeImageSteganographer {
    ...
    fn view_decoded_image_temp(&self,
        ...
        // Calculate window size (with reasonable limits)
        let target_width = if img_width > 1600 { 1600 } else { img_width };
        let scale_factor = target_width as f32 / img_width as f32;
        let target_height = (img_height as f32 * scale_factor) as u32;
       
        // Convert to RGB and prepare buffer
        let rgb_img = if scale_factor != 1.0 {
            img.resize(target_width, target_height, image::imageops::FilterType::Triangle).to_rgb()
        } else {
            img.to_rgb()
        };
        ...
        let mut window = Window::new(
            "Image Viewer - Press Escape to close",
